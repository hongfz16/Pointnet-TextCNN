...
sys.path.append(os.path.join(ROOT_DIR, 'Pointnet_TextCNN'))
...
parser.add_argument('--model_path', default='/data/hongfz/modelnet40/log/model.ckpt', help='model checkpoint file path [default: log/model.ckpt]')
parser.add_argument('--dump_dir', default='/data/hongfz/modelnet40/dump', help='dump folder path [dump]')
...
SHAPE_NAMES = [line.rstrip() for line in \
    open(os.path.join('/data/hongfz/modelnet40/', 'data/modelnet40_ply_hdf5_2048/shape_names.txt'))] 
...
if FLAGS.normal:
    ...
    TRAIN_DATASET = modelnet_h5_dataset.ModelNetH5Dataset(os.path.join('/data/hongfz/modelnet40/', 'data/modelnet40_ply_hdf5_2048/train_files.txt'), batch_size=BATCH_SIZE, npoints=NUM_POINT, shuffle=True)
    TEST_DATASET = modelnet_h5_dataset.ModelNetH5Dataset(os.path.join('/data/hongfz/modelnet40/', 'data/modelnet40_ply_hdf5_2048/test_files.txt'), batch_size=BATCH_SIZE, npoints=NUM_POINT, shuffle=False)
...
def evaluate(num_votes):
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        pred= MODEL.get_model(384, [8, 27, 64, 125], pointclouds_pl, is_training_pl, [1, 2, 3], 32)
