...
def get_textcnn_model(text_embedding, filter_sizes, filter_num, is_training, bn_decay=None):
    ...
    for i, filter_size in enumerate(filter_sizes):
        ...
        text_embedding = tf.expand_dim(text_embedding, -1)
        ...
        conved = tf_util.conv2d(text_embedding, filter_num, [filter_size, embedding_size],
                                ...
                                is_training = is_training, data_format = 'NCHW')
    ...
    concated = tf.concat(1, pooled_output)
    return concated
...
def get_model(sample_num, sample_scale, point_cloud, is_training, filter_sizes, filter_num, bn_decay=None):
    ...
    for i, scale in enumerate(sample_scale):
        ...
        grouped_xyz = group_point(point_cloud, idx)
        for j, channel in enumerate(channels):
            # [batch, sample_num, scale, channel]
            points_features = tf_util.conv2d(grouped_xyz, channel, [1, 1],
                                             padding = 'VALID', stride = [1, 1],
                                             bn = True, is_training = is_training,
                                             scope='conv_%d'%(j), bn_decay = bn_decay,
                                             data_format = 'NCHW')
        ...
        # [batch, sample_num, 1, 128]
        points_features = tf.expand_dim(points_features, 2)
        # [batch * sample_num, 1, 128]
    ...
    textcnn_embedding = tf.concat(1, feature_collection)
    ...
    textcnn_encoded = tf.expand_dim(textcnn_encoded, 2)
    channels = [256, 512, 1024]
    for i, channel in enumerate(channels):
        # [batch, sample_num, 1, channel]
        global_feature = tf_util.conv2d(textcnn_encoded, channel, [1, 1],
                                        padding = 'VALID', stride = [1, 1],
                                        bn = True, is_training = is_training,
                                        scope='feature_aggregation_conv_%d'%(j), bn_decay = bn_decay,
                                        data_format = 'NCHW')
    # [batch, 1, 1, 1024]
